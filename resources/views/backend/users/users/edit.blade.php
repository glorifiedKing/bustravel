...
<div class="container-fluid">
    <div class="row">
        ...
        <div class="card">
            ...
            <div class="row">
              <div class="col-md-12">
              <form role="form" action="{{route('bustravel.users.update',$user->id)}}" method="POST" >
              {{csrf_field() }}
              ...
              <div class="box-body">
                  <div class="row">
                    ...
                    <div class="form-group col-md-3  ">
                        ...
                        <input type="text"  name="phone_number" value="{{$user->phone_number}}" class="form-control {{ $errors->has('phone_number') ? ' is-invalid' : '' }}" id="exampleInputEmail1" placeholder="Enter Phone Number"  >
                    ...
                    <div class="form-group col-md-3">
                         ...
                         <select class="form-control select2 {{ $errors->has('operator_id') ? ' is-invalid' : '' }}" name="operator_id"  placeholder="Select Operator" >
                           ...
                           @foreach($operators as $operator)
                               <option value="{{$operator->id}}"  @php echo $user->operator_id == $operator->id ? 'selected' :  "" @endphp>{{$operator->name}}</option>
                           @endforeach
                         </select>
                    ...
                    <div class="form-group col-md-3">
                        ...
                        <select class="form-control select2 {{ $errors->has('workstation') ? ' is-invalid' : '' }}" name="workstation"  placeholder="Select Operator" >
                          ...
                          @foreach($stations as $station)
                              <option value="{{$station->id}}" {{($user->workstation == $station->id)? 'selected' : ''}}>{{$station->name}} ( {{$station->code}} )</option>
                          @endforeach
                        </select>
                    ...
                    <div class=" col-md-3 form-group">
                        ...
                        <label class="radio-inline">
                          <input type="radio" id="Active" name="status" value="1" @php echo $user->status == 1 ? 'checked' :  "" @endphp> Active</label>
                        </label>
                       <label class="radio-inline">
                          <input type="radio" id="Deactive" name="status" value="0"@php echo $user->status == 0 ? 'checked' :  "" @endphp > Deactive</label>
                       </label>
                    </div>
                  </div>
              </div>
