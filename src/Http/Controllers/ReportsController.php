  ...
  //Profitable Route Report
   public function routes()
   {
     if(request()->isMethod('post'))
      ...
      if($period ==1)
      {
        ...
        $weekStartDate = $now->startOfWeek()->format('Y-m-d ');
        $weekEndDate = $now->endOfWeek()->format('Y-m-d');
        $daysales =Booking::select(\DB::raw('sum(amount) as total,routes_departure_time_id'))->whereBetween('created_at', array($weekStartDate." 00:00:00", $weekEndDate." 23:59:59"))->where('status',1)
        ...
        foreach ($daterange as $weekdate) {
         ...
         foreach($weekdates as $wdates)
         {
            $daysales1 =Booking::whereBetween('created_at', array($wdates." 00:00:00", $wdates." 23:59:59"))->where('status',1)->where('routes_departure_time_id',$first_route)->sum('amount');
            $y_axis1[] = $daysales1;
            $daysales2 =Booking::whereBetween('created_at', array($wdates." 00:00:00", $wdates." 23:59:59"))->where('status',1)->where('routes_departure_time_id',$second_route)->sum('amount');
            $y_axis2[] = $daysales2;
            $daysales3 =Booking::whereBetween('created_at', array($wdates." 00:00:00", $wdates." 23:59:59"))->where('status',1)->where('routes_departure_time_id',$third_route)->sum('amount');
            $y_axis3[] = $daysales3;
         }
       }elseif($period==2)
       {
         ...
         $monthStartDate = $now->startOfMonth()->format('Y-m-d ');
         $monthEndDate = $now->endOfMonth()->format('Y-m-d');
         $daysales =Booking::select(\DB::raw('sum(amount) as total,routes_departure_time_id'))->whereBetween('created_at', array($monthStartDate." 00:00:00", $monthEndDate." 23:59:59"))->where('status',1)
         ->groupBy('routes_departure_time_id')
         ...
         foreach ($daterange as $monthdate) {
          ...
          foreach($monthdates as $mdates)
          {
          $daysales1 =Booking::whereBetween('created_at', array($mdates." 00:00:00", $mdates." 23:59:59"))->where('status',1)->where('routes_departure_time_id',$first_route)->sum('amount');
          $y_axis1[] = $daysales1;
          $daysales2 =Booking::whereBetween('created_at', array($mdates." 00:00:00", $mdates." 23:59:59"))->where('status',1)->where('routes_departure_time_id',$second_route)->sum('amount');
          $y_axis2[] = $daysales2;
          $daysales3 =Booking::whereBetween('created_at', array($mdates." 00:00:00", $mdates." 23:59:59"))->where('status',1)->where('routes_departure_time_id',$third_route)->sum('amount');
          $y_axis3[] = $daysales3;
          }
        }elseif($period==3)
        {
        ...
        }elseif($period==4)
        {
          ...
          ->whereBetween('created_at', array($endperiod->format('Y-m-d') ." 00:00:00", $startperiod->format('Y-m-d') ." 23:59:59"))->where('status',1)
          ->groupBy('routes_departure_time_id')
          ->orderBY('total','DESC')->limit(3)->get();
          $first_route = $daysales[0]->routes_departure_time_id??0;
          $second_route = $daysales[1]->routes_departure_time_id??0;
          ...
          $monthStartDate = $monthdatestring->startOfMonth()->format('Y-m-d');
          $monthEndDate =   $monthdatestring->endOfMonth()->format('Y-m-d');
          $daysales1 =Booking::whereBetween('created_at', array($monthStartDate." 00:00:00", $monthEndDate." 23:59:59"))->where('status',1)->where('routes_departure_time_id',$first_route)->sum('amount');
          $y_axis1[] = $daysales1;
          $daysales2 =Booking::whereBetween('created_at', array($monthStartDate." 00:00:00", $monthEndDate." 23:59:59"))->where('status',1)->where('routes_departure_time_id',$second_route)->sum('amount');
          $y_axis2[] = $daysales2;
          $daysales3 =Booking::whereBetween('created_at', array($monthStartDate." 00:00:00", $monthEndDate." 23:59:59"))->where('status',1)->where('routes_departure_time_id',$third_route)->sum('amount');
          $y_axis3[] = $daysales3;
          }
...

         {
         ...
         }elseif($period==6)
           {
             ...
             $yearEndDate = \Carbon\Carbon::parse('last day of December');
             $daysales =Booking::select(\DB::raw('sum(amount) as total,routes_departure_time_id'))
             ->whereBetween('created_at', array($yearStartDate->format('Y-m-d') ." 00:00:00", $yearEndDate->format('Y-m-d') ." 23:59:59"))->where('status',1)
             ->groupBy('routes_departure_time_id')
             ->orderBY('total','DESC')->limit(3)->get();
             $first_route = $daysales[0]->routes_departure_time_id??0;
             $second_route = $daysales[1]->routes_departure_time_id??0;
   ...
   //Traffic Report
      ...
      if(request()->isMethod('post'))
       ...
       if($period ==1)
       {
         ...
         foreach ($daterange as $weekdate) {
          ...
          foreach($weekdates as $wdates)
          {
             $daysales =Booking::whereBetween('created_at', array($wdates." 00:00:00", $wdates." 23:59:59"))->where('status',1)->count();
          $y_axis[] = $daysales;
          }
        }elseif($period==2)
        {
          ...
          foreach ($daterange as $monthdate) {
           ...
           foreach($monthdates as $mdates)
           {
              $daysales =Booking::whereBetween('created_at', array($mdates." 00:00:00", $mdates." 23:59:59"))->where('status',1)->count();
           $y_axis[] = $daysales;
           }
         }elseif($period==3)
         {
         ...
         }elseif($period==4)
         {
           ...
           $monthStartDate = $monthdatestring->startOfMonth()->format('Y-m-d');
           $monthEndDate =   $monthdatestring->endOfMonth()->format('Y-m-d');
           $monthsales =Booking::whereBetween('created_at', array($monthStartDate." 00:00:00", $monthEndDate." 23:59:59"))->where('status',1)->count();
           $y_axis[] =$monthsales;
           }
    ...
    if(!is_null($ticket))
    {
     ...
     $bookings = Booking::whereBetween('created_at', array($from." 00:00:00", $to." 23:59:59"))->orderBY('id','DESC')->get();
   }
